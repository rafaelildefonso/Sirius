...
def _click_first_profile_by_screenshot() -> bool:
    ...
    try:
        ...
        print(f"[GameUpdater] [TARGET] Profil avatarı ({abs_x}, {abs_y}) — tıklanıyor")
...
def _handle_steam_profile_selection() -> bool:
    ...
    try:
        ...
        if not is_small and white_pixels <= 100:
            print("[GameUpdater] [i] Profil dialogu yok - Steam zaten giriş yapmış")
            return False
    except ImportError:
        pass
    except Exception:
    ...
    print("[GameUpdater] [USER] Profil seçimi tespit edildi — ilk profile tıklanıyor")
...
def _handle_install_dialog(game_name: str) -> str:
    ...
    print(f"[GameUpdater] [TROPHY] Hedef sürücü: {drive_label} ({best_drive['free_gb']:.1f} GB boş)")
...
def _ensure_steam_running(steam_path: Path) -> bool:
    ...
    if not exe.exists():
        print(f"[GameUpdater] [FAIL] Steam bulunamadı: {exe}")
        return False

    print("[GameUpdater] [RUN] Steam başlatılıyor...")
    if is_mac():
...
def _search_steam_appid(game_name: str) -> tuple[str | None, str | None]:
    ...
    if name_lower in _KNOWN_APPIDS:
        ...
        print(f"[GameUpdater] [BOOK] Bilinen: {canonical} ({app_id})")
        return app_id, canonical

    for key, (app_id, canonical) in _KNOWN_APPIDS.items():
        if name_lower in key or key in name_lower:
            print(f"[GameUpdater] [BOOK] Kısmi eşleşme: {canonical} ({app_id})")
            return app_id, canonical
...
def _schedule_mac(hour: int, minute: int) -> str:
    ...
    plist_path  = plist_dir / "com.sirius.gameupdater.plist"
    script_path = Path(__file__).resolve()
    plist_content = f"""<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN"
  "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0"><dict>
    <key>Label</key><string>com.sirius.gameupdater</string>
...
def _cancel_scheduled_update() -> str:
    if is_windows():
        result = subprocess.run(
            ["schtasks", "/Delete", "/TN", "SIRIUS_GameUpdater", "/F"],
            capture_output=True, text=True
        )
        return ("Scheduled update cancelled."
                if result.returncode == 0 else "No scheduled update found.")
    if is_mac():
        plist_path = Path.home() / "Library" / "LaunchAgents" / "com.sirius.gameupdater.plist"
...
def _get_schedule_status() -> str:
    if is_windows():
        result = subprocess.run(
            ["schtasks", "/Query", "/TN", "SIRIUS_GameUpdater", "/FO", "LIST"],
            capture_output=True, text=True
        )
        if result.returncode != 0:
...
if __name__ == "__main__":
    if "--scheduled" in sys.argv:
        print(f"[GameUpdater] [TIME] Scheduled run at {datetime.now().strftime('%H:%M')}")
