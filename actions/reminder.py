...
def _schedule_windows(target_dt: datetime, task_name: str,
    ...
    if result.returncode != 0:
        ...
        print(f"[Reminder] [FAIL] schtasks: {err}")
        return ""  
...
def _schedule_mac(target_dt: datetime, task_name: str,
    ...
    label     = f"com.sirius.reminder.{task_name}"
...
"""
    ...
    if result.returncode != 0:
        ...
        script_path.unlink(missing_ok=True)
        print(f"[Reminder] [FAIL] launchctl: {result.stderr.strip()}")
        return ""
...
def _schedule_linux(target_dt: datetime, task_name: str,
    ...
    if shutil.which("at"):
        ...
        if result.returncode == 0:
            return task_name
        print(f"[Reminder] [FAIL] at: {result.stderr.strip()}")
        return ""

    print("[Reminder] [FAIL] Neither systemd-run nor at found on this Linux system.")
    return ""
...
def reminder(
    ...
    task_name  = f"SIRIUSReminder_{target_dt.strftime('%Y%m%d_%H%M%S')}"
    ...
    try:
        ...
        script_path.unlink(missing_ok=True)
        print(f"[Reminder] [FAIL] Scheduling exception: {e}")
