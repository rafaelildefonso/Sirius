...
def _system_open(url: str, browser_exe: str | None = None):
    ...
    try:
        ...
        print(f"[Browser] System open failed: {e}")
        return False
...
def _find_exe_windows(prog_name: str) -> Optional[str]:
    ...
    # Common manual paths if registry fails
    if _OS == "Windows":
        local = os.environ.get("LOCALAPPDATA", "")
        prog  = os.environ.get("PROGRAMFILES", "")
        prog86 = os.environ.get("PROGRAMFILES(X86)", "")
        ...
        common = {
            "brave": [
                ...
                Path(prog86) / "BraveSoftware" / "Brave-Browser" / "Application" / "brave.exe",
        ...
        for p in common.get(prog_name.lower(), []):
            if p.exists():
                return str(p)
...
def _detect_default_browser() -> str:
    ...
    # Check for RUNNING browsers if we can't find default
    for b in ("brave", "chrome", "edge", "firefox"):
        spec = _BROWSER_SPECS.get(_OS, {}).get(b)
        if spec:
            for bin_name in spec.get("bins", []):
                if _is_running(bin_name):
                    return b
...
class _BrowserSession:
    ...
    def _is_alive(self) -> bool:
        """Checks if the current context and page are alive."""
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using SIRIUS profile")
                sirius = str(Path.home() / ".sirius_profiles" / "firefox_sirius")
        ...
        try:
            ctx = await engine_obj.launch_persistent_context(profile, **kwargs)
            ...
            try:
                ctx = await engine_obj.launch_persistent_context(sirius_profile, **kwargs)
                await asyncio.sleep(0.2)
                pg = await ctx.new_page()
                self._context = ctx
                self._page    = pg
    ...
    async def go_to(self, url: str) -> str:
        ...
        try:
            ...
            # If launch failed (likely profile locked), try system open
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / "sirius_screenshot.png")
...
class _SessionRegistry:
    ...
    def get(self, browser_name: str | None = None) -> _BrowserSession:
        if not browser_name:
            ...
            for b in ("brave", "chrome", "edge", "firefox"):
                spec = _resolve_browser(b)
                if spec and spec.get("exe"):
                    exe_file = Path(spec["exe"]).name
